...
VALUES
    ...
    (
        "kmom06",
        "niklas@bth.se",
        "Det är rätt skönt att det finns fler typer av databaser att jobba mot när det kommer till webbutveckling. I nuläget så tycker jag om MySQL mer är MongoDB men det är härligt hur simpelt och snabbt man kan komma igång med MongoDB, speciellt i den här typen av projekt. I framtiden kommer jag nog använda båda ändå, det verkar som många seriösa företag nyttjar bägge typ av databaser för att hantera deras data. Jag vet inte riktigt hur stor skillnaden mellan MySQL och SQLite3 är i nuläget men jag kan anta att det har en del med optimering och hastighet att göra. Med tanke på att det är så portabelt och inte kräver någon installation så måste jag nästan anta att den inte att fullt lika snabb och har svårare att skala uppåt. SQLite3 saknar även möjligt för att optimera sig till en vis grad. Man kan till exempel inte definiera procedures eller använda alla typer av funktioner som kommer tillsammans med MySQL. 
        

        Vid det här laget så känner jag mig rätt bekväm i React och dess funktionalitet. Jag är mindre nöjd med hur strukturen på applikationen ser ut just nu och är lite för stressad för att justera det och göra det bättre och väljer istället att lägga ner arbetstimmarna på kraven istället. Just nu ser jag inte riktigt hur asynkron programmering ska hjälpa mig vid mindre projekt så som detta. All information som jag hämtar via promises gör att nästa block av kod ska bli påverkad. Det går inte riktigt att vänta på data och att påbörja callback-hell är inte roligt. Jag misstänker att det kommer till mer hjälp vid större projekt där man inte eller baserar nästa rader kod på resultatet av förfrågan som skickades.
        

        Docker återigen möter alla mina förväntningar och lite till. Inte nog med att det kan användas för att styra upp tester för olika miljöer och versioner av miljöer men vi kan även installera verktyg för att se till att mjukvaror beter sig som vi tänkte. Mycket bra exempel denna vecka då vi installerar Mongo i terminalen och spinner upp en databas som det går att bygga på via, i det här fallet, Socketio. Om man tidigare har jobbat med objekt i någon typ av form eller har erfarenhet av JSON så är transaktionen till MongoDB väldigt enkel. I nuläget så ser jag inte någon större skillnad på dom bortsätt från dom självklara funktioner som kommer direkt ur lådan. Det skrämmer mig lite hur ostrukturerat det kan blir om en användare, eller vad man nu sparar, har en lång lista med information. Om man själv inte definierat en maxlängd på den listan så kan den hålla väldigt mycket information vilket kan se rörigt ut. Kanske en erfaren utvecklare väljer att sortera bort den typ av data om den förväntas kunna bli stor så att en förfrågan inte tar så lång tid att göra. Allt som allt är jag glad att det finns många alternativ så att utvecklarna indirekt pressar varandra att förbättra och optimera respektive databas för oss utvecklare."
    )
;